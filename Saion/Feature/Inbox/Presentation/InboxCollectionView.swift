...
final class InboxCollectionView: UICollectionView {
    ...
    typealias Item = InboxCellItem
    
    // MARK: Registrations
    
    /// 알림 아이템을 알림 셀로 구성하는 데이터 소스 레지스트레이션
    private let inboxItemCellRegistration =
    CellRegistration<InboxCell, Item> { cell, _, item in
    ...
    UICollectionViewDiffableDataSource<Section, Item>(collectionView: self) {
        ...
        return dequeueConfiguredReusableCell(
            using: inboxItemCellRegistration,
            for: indexPath,
            item: item
        )
    }
