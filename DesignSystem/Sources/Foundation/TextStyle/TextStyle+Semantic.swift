//
//  TextStyle+Semantic.swift
//  DesignSystem
//
//  Created by 신정욱 on 6/12/26.
//

import UIKit

extension TextStyle.Typography {
    public static let display1 = TextStyle.Typography(
        font: .pretendard(size: .px40, weight: .bold),
        lineHeight: .px48,
        letterSpacing: .tight
    )
    
    public static let display2 = TextStyle.Typography(
        font: .pretendard(size: .px32, weight: .bold),
        lineHeight: .px40,
        letterSpacing: .tight
    )
    
    public static let heading1 = TextStyle.Typography(
        font: .pretendard(size: .px24, weight: .bold),
        lineHeight: .px31,
        letterSpacing: .normal
    )
    
    public static let heading2 = TextStyle.Typography(
        font: .pretendard(size: .px20, weight: .bold),
        lineHeight: .px28,
        letterSpacing: .normal
    )
    
    public static let title1 = TextStyle.Typography(
        font: .pretendard(size: .px18, weight: .semiBold),
        lineHeight: .px25,
        letterSpacing: .normal
    )
    
    public static let title2 = TextStyle.Typography(
        font: .pretendard(size: .px16, weight: .semiBold),
        lineHeight: .px22,
        letterSpacing: .normal
    )
    
    public static let title3 = TextStyle.Typography(
        font: .pretendard(size: .px15, weight: .medium),
        lineHeight: .px22,
        letterSpacing: .normal
    )
    
    public static let body1 = TextStyle.Typography(
        font: .pretendard(size: .px16, weight: .medium),
        lineHeight: .px24,
        letterSpacing: .normal
    )
    
    public static let body2 = TextStyle.Typography(
        font: .pretendard(size: .px14, weight: .regular),
        lineHeight: .px21,
        letterSpacing: .normal
    )
    
    public static let body3 = TextStyle.Typography(
        font: .pretendard(size: .px13, weight: .regular),
        lineHeight: .px20,
        letterSpacing: .normal
    )
    
    public static let label1 = TextStyle.Typography(
        font: .pretendard(size: .px14, weight: .semiBold),
        lineHeight: .px20,
        letterSpacing: .normal
    )
    
    public static let label2 = TextStyle.Typography(
        font: .pretendard(size: .px12, weight: .semiBold),
        lineHeight: .px17,
        letterSpacing: .normal
    )
    
    public static let caption1 = TextStyle.Typography(
        font: .pretendard(size: .px12, weight: .regular),
        lineHeight: .px17,
        letterSpacing: .normal
    )
    
    public static let caption2 = TextStyle.Typography(
        font: .pretendard(size: .px11, weight: .regular),
        lineHeight: .px16,
        letterSpacing: .wide
    )
    
    public static let title1Subtle = TextStyle.Typography(
        font: .pretendard(size: .px18, weight: .medium),
        lineHeight: .px25,
        letterSpacing: .normal
    )
    
    public static let title1Strong = TextStyle.Typography(
        font: .pretendard(size: .px18, weight: .bold),
        lineHeight: .px25,
        letterSpacing: .normal
    )
    
    public static let heading1Subtle = TextStyle.Typography(
        font: .pretendard(size: .px22, weight: .medium),
        lineHeight: .px31,
        letterSpacing: .normal
    )
    
    public static let title3Strong = TextStyle.Typography(
        font: .pretendard(size: .px15, weight: .semiBold),
        lineHeight: .px22,
        letterSpacing: .normal
    )
    
    public static let label1Strong = TextStyle.Typography(
        font: .pretendard(size: .px14, weight: .bold),
        lineHeight: .px20,
        letterSpacing: .normal
    )
    
    public static let label1Subtle = TextStyle.Typography(
        font: .pretendard(size: .px14, weight: .medium),
        lineHeight: .px21,
        letterSpacing: .normal
    )
    
    public static let label2Strong = TextStyle.Typography(
        font: .pretendard(size: .px12, weight: .bold),
        lineHeight: .px17,
        letterSpacing: .normal
    )
    
    public static let caption2Strong = TextStyle.Typography(
        font: .pretendard(size: .px11, weight: .semiBold),
        lineHeight: .px14,
        letterSpacing: .wide
    )
}
